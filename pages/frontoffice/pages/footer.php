    ...
    <div>
      ...
      <ul>
        <?php foreach (array_slice($categories, 0, 5) as $cat): ?>
          <li><a
              href="/frontoffice/?page=category&category=<?php echo urlencode($cat['name']); ?>"><?php echo htmlspecialchars($cat['name']); ?></a>
          </li>
        <?php endforeach; ?>
      </ul>
    </div>
...
<div class="mobile-only">
  ...
  <?php if ($page === 'home'): ?>
    ...
    <div class="breaking-carousel">
      <?php foreach ($breakingNews as $news): ?>
        <div class="breaking-card hover-card"
          onclick="navigate('/pages/frontoffice/?page=article&slug=<?php echo urlencode($news['slug'] ?? $news['id']); ?>')">
          <img
            src="<?php echo htmlspecialchars($news['image'] ?? 'https://images.unsplash.com/photo-1517649763962-0c623066013b?w=600&q=80'); ?>"
    ...
    <?php foreach (array_slice($latestArticles, 0, 3) as $rec): ?>
      <div class="mob-rec-card"
        onclick="navigate('/pages/frontoffice/?page=article&slug=<?php echo urlencode($rec['slug'] ?? $rec['id']); ?>')">
        <img class="thumb"
          src="<?php echo htmlspecialchars($rec['image'] ?? 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=300&q=80'); ?>"
    ...
    <div class="mobile-only">
      ...
      <div class="mobile-art-page">
        <?php if (!empty($article['images'])): ?>
          <img class="mob-art-hero" src="<?php echo htmlspecialchars($article['images'][0]['image_url']); ?>"
            alt="<?php echo htmlspecialchars($article['title']); ?>" />
        <?php else: ?>
          <img class="mob-art-hero" src="https://images.unsplash.com/photo-1517649763962-0c623066013b?w=800&q=80"
            alt="<?php echo htmlspecialchars($article['title']); ?>" />
        <?php endif; ?>
  ...
  <?php elseif ($page === 'discover'): ?>
    ...
    <div class="mobile-only">
      <header class="mobile-header">
        <button class="back-btn" style="background:none;border:none;cursor:pointer;" onclick="goBack()"><i
            class="fas fa-chevron-left"></i></button>
        <span style="font-weight:700;font-size:15px;">Discover</span>
        <div></div>
      </header>

      <div style="padding:20px 20px 10px;">
        <h1 style="font-family:var(--font-display);font-size:2rem;">Discover</h1>
        <p style="color:var(--gray-mid);font-size:14px;margin-bottom:18px;">News from all around the world</p>
        
        <div class="cat-filters" style="margin-bottom:4px;">
          <button class="cat-btn <?php echo !$currentCategory ? 'active' : ''; ?>"
            onclick="navigate('/frontoffice/?page=discover')">All</button>
          <?php foreach (array_slice($categories, 0, 5) as $cat): ?>
            <button class="cat-btn <?php echo ($currentCategory == $cat['name']) ? 'active' : ''; ?>"
              onclick="navigate('/frontoffice/?page=category&category=<?php echo urlencode($cat['name']); ?>')"><?php echo htmlspecialchars($cat['name']); ?></button>
          <?php endforeach; ?>
        </div>
      </div>

      <div class="discover-list" style="padding: 0 20px;">
        <?php foreach (array_slice($articles, 0, 10) as $article): ?>
          <div class="discover-card"
            onclick="navigate('/frontoffice/?page=article&slug=<?php echo urlencode($article['slug'] ?? $article['id']); ?>')"
            style="cursor:pointer;">
            <img class="thumb"
              src="<?php echo htmlspecialchars($article['image'] ?? 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=300&q=80'); ?>"
              alt="<?php echo htmlspecialchars($article['title']); ?>" />
            <div class="info">
  ...
  <?php else: ?>
    <footer class="site-footer">
      <div class="footer-grid">
        <div>
          <div class="brand">VERTO<span>NEWS</span></div>
          <p>Your trusted source for breaking news, in-depth analysis, and stories that matter — from around the world.
          </p>
        </div>
        <div>
          <h5>Categories</h5>
